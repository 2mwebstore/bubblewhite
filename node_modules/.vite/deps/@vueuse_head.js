import { Kn as ref, Nt as onDeactivated, Ot as nextTick, _n as watchEffect, gn as watch, jt as onBeforeUnmount, kt as onActivated, mn as version, nr as unref, nt as defineComponent, ut as getCurrentInstance, xt as inject } from "./vue.runtime.esm-bundler-D4M2X7UN.js";
	...
	for (let i = startIndex; i < keys.length; i += 1) {
		...
		if (!virtual && !TagConfigKeys.has(k)) {
			...
			if (v === "true" || v === "") props[k] = isDataKey ? "true" : true;
				...
				if (isDataKey && v === "false") props[k] = "false";
				else delete props[k];
			}
		}
	}
	...
	Object.keys(input).forEach((key) => {
		...
		switch (key) {
			...
			case "noscript":
				...
				break;
			case "script": if (Array.isArray(tagValue)) filtered[key] = tagValue.map((script) => {
				const safeScript = acceptDataAttrs(script);
				WhitelistAttributes.script.forEach((s) => {
					if (script[s]) {
						if (s === "textContent") try {
...
var XSSPlugin = defineHeadPlugin({ hooks: { "tags:afterResolve": (ctx) => {
	for (const tag of ctx.tags) if (typeof tag.innerHTML === "string") {
		if (tag.innerHTML && (tag.props.type === "application/ld+json" || tag.props.type === "application/json")) tag.innerHTML = tag.innerHTML.replace(/</g, "\\u003C");
...
var unheadVueComposablesImports = { "@unhead/vue": [...["injectHead"], ...composableNames] };
